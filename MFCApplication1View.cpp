...
void CMFCApplication1View::hisEqualiz(BYTE* image, int w, int h, BYTE* outImg)
{
	...
	BYTE gray[256];
	float cons;
	cons = static_cast<float>(255) / his[255];
	for (n = 0; n < 256; n++)
	{
		...
		for (j = 0; j < width; j++)
		{
			if (i == 0 || j == 0 || i == heigth - 1 || j == width - 1)
			{
				...
				for (m = -1; m < 2; m++)
				{
					...
					outImg[i*width+j] = (BYTE)(value/ 9.0);
				}
		}
	}
...
void CMFCApplication1View::gaussian(BYTE* image, int width, int heigth, BYTE* outImg)
{
	...
	smth[0] = 1; smth[1] = 2; smth[2] = 1;
	smth[3] = 2; smth[4] = 4; smth[5] = 2;
	smth[6] = 1; smth[7] = 2; smth[8] = 1;
	int value;
	for (i = 0; i < heigth; i++)
	{
		for (j = 0; j < width; j++)
		{
			if (i == 0 || j == 0 || i == heigth - 1 || j == width - 1)
			{
				...
				for (m = -1; m < 2; m++)
				{
					...
					value = convolution(smth, block);
					outImg[i * width + j] = (BYTE)(value / 16.0);
				}
		}
	}
}
		...
		for (j = 0; j < width; j++)
		{
			...
			else
			{
				for (m = -1; m < 2; m++)
				{
					for (n = -1; n < 2; n++)
					{
						block[(m + 1) * 3 + n + 1] = image[(i + m + 1) * heigth + n + j + 1 ];
					}
				}
				value = MidValueFind(blockNum, block);
				outImg[i * heigth + j] = value;
			}
		}
	}
}
